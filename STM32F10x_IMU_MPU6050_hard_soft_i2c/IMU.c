...
uint8_t offset_received_data[12];

float accel_x, accel_y, accel_z, gyro_x, gyro_y,gyro_z;
float temp;	

int16_t data_x[2], data_y[2],  data_z[2];
int16_t data_ox[2], data_oy[2],  data_oz[2];
...
void IMU_get_accel(void){
		accel_x = IMU_get_accel_X() - off_set_ax;
		accel_y = IMU_get_accel_Y()	- off_set_ay;
		accel_z = IMU_get_accel_Z() - off_set_az;
}


void IMU_get_gyro(void){
		gyro_x = IMU_get_gyro_X() - off_set_gx;
		gyro_y = IMU_get_gyro_Y()  - off_set_gy;
		gyro_z = IMU_get_gyro_Z() - off_set_gz;
}
...
void IMU_calculate_offset(void){
	...
	for(int i=0;i<f;i++){
			...
			off_set_ax += IMU_get_accel_X();
			off_set_ay += IMU_get_accel_Y();
			off_set_az += (IMU_get_accel_Z()-1);
		
			off_set_gx += IMU_get_gyro_X();
			off_set_gy += IMU_get_gyro_Y();
			off_set_gz += IMU_get_gyro_Z();
	}
...
void IMU_soft_i2c_get_temp(void){
	received_data[12] = soft_i2c_single_read(IMU_soft_I2C_default_Address,IMU_TMP_OUT_H);
	received_data[13] = soft_i2c_single_read(IMU_soft_I2C_default_Address,IMU_TMP_OUT_L);
	temp = (int16_t) ((received_data[12]<<8|received_data[13]));
	temp = (float) temp/(340.00)+36.53;
}

float  IMU_soft_I2C_get_accel_X(void){
					received_data[0] = soft_i2c_single_read(IMU_soft_I2C_default_Address,IMU_ACCEL_XOUT_H);
					received_data[1] = soft_i2c_single_read(IMU_soft_I2C_default_Address,IMU_ACCEL_XOUT_L);
					data_x[0] =(int16_t)  ((received_data[0] <<8| received_data[1]) );
					accel_x = data_x[0]/IMU_ACCEL_SENS_8;
					return(accel_x);
...
float  IMU_soft_I2C_get_accel_Y(void){
					received_data[2] = soft_i2c_single_read(IMU_soft_I2C_default_Address,IMU_ACCEL_YOUT_H);
					received_data[3] = soft_i2c_single_read(IMU_soft_I2C_default_Address,IMU_ACCEL_YOUT_L);
					data_y[0] =(int16_t)  ((received_data[2] <<8| received_data[3]) );		
					accel_y = data_y[0]/IMU_ACCEL_SENS_8;
					return(accel_y);
...
float  IMU_soft_I2C_get_accel_Z(void){
					received_data[4] = soft_i2c_single_read(IMU_soft_I2C_default_Address,IMU_ACCEL_ZOUT_H);
					received_data[5] = soft_i2c_single_read(IMU_soft_I2C_default_Address,IMU_ACCEL_ZOUT_L);
					data_z[0] =(int16_t)  ((received_data[4] <<8| received_data[5]) );
					accel_z = data_z[0]/IMU_ACCEL_SENS_8;
					return(accel_z);
...
float  IMU_soft_I2C_get_gyro_X(void){
					received_data[6] = soft_i2c_single_read(IMU_soft_I2C_default_Address,IMU_GYRO_XOUT_H);
					received_data[7] = soft_i2c_single_read(IMU_soft_I2C_default_Address,IMU_GYRO_XOUT_L);
					data_x[1] =(int16_t)  ((received_data[6] <<8| received_data[7]) );
					gyro_x = data_x[1] / IMU_GYRO_SENS_500;
					return(gyro_x);
...
float  IMU_soft_I2C_get_gyro_Y(void){
					received_data[8] = soft_i2c_single_read(IMU_soft_I2C_default_Address,IMU_GYRO_YOUT_H);
					received_data[9] = soft_i2c_single_read(IMU_soft_I2C_default_Address,IMU_GYRO_YOUT_L);
					data_y[1] =(int16_t)  ((received_data[8] <<8| received_data[9]) );
					gyro_y = data_y[1] / IMU_GYRO_SENS_500;
					return(gyro_y);
...
float  IMU_soft_I2C_get_gyro_Z(void){
					received_data[10] = soft_i2c_single_read(IMU_soft_I2C_default_Address,IMU_GYRO_ZOUT_H);
					received_data[11] = soft_i2c_single_read(IMU_soft_I2C_default_Address,IMU_GYRO_ZOUT_L);
					data_z[1] =(int16_t)  ((received_data[10] <<8| received_data[11]) );
					gyro_z = data_z[1] / IMU_GYRO_SENS_500;
					return(gyro_z);
...
void IMU_soft_I2C_get_accel(void){
		accel_x = IMU_soft_I2C_get_accel_X() - off_set_ax;
		accel_y = IMU_soft_I2C_get_accel_Y()	- off_set_ay;
		accel_z = IMU_soft_I2C_get_accel_Z() - off_set_az;
}


void IMU_soft_I2C_get_gyro(void){
		gyro_x = IMU_soft_I2C_get_gyro_X() - off_set_gx;
		gyro_y = IMU_soft_I2C_get_gyro_Y()  - off_set_gy;
		gyro_z = IMU_soft_I2C_get_gyro_Z() - off_set_gz;
}
...
void IMU_soft_I2C_calculate_offset(void){
	...
	for(int i=0;i<f;i++){
			...
			off_set_ax += IMU_soft_I2C_get_accel_X();
			off_set_ay += IMU_soft_I2C_get_accel_Y();
			off_set_az += (IMU_soft_I2C_get_accel_Z()-1);
		
			off_set_gx += IMU_soft_I2C_get_gyro_X();
			off_set_gy += IMU_soft_I2C_get_gyro_Y();
			off_set_gz += IMU_soft_I2C_get_gyro_Z();
	}
			off_set_ax = (float) off_set_ax/f;
			off_set_ay = (float) off_set_ay/f;
			off_set_az = (float) off_set_az/f;
